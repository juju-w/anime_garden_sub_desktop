...
import { Plus, Trash2, Loader2, Globe, Edit3, Copy, Check } from 'lucide-react';
...
const translations = {
  en: {
    ...
    ariaSecret: "Token",
    storage: "Downloads are managed by your local Aria2 config.",
    ...
    status_pending: "Wait",
    settingsSaved: "Saved!",
    connectionTip: "Using local system for processing.",
    copyAll: "Copy All",
    exportTxt: "Export .txt",
    copied: "Copied!",
    clearHistory: "Clear History",
    confirmClear: "Clear all records? (Irreversible)",
    systemInfo: "Desktop Edition",
  ...
  cn: {
    ...
    status_pending: "等待",
    settingsSaved: "已保存！",
    connectionTip: "提示：桌面版直接使用本地资源解析。",
    copyAll: "复制全部磁力链",
    exportTxt: "导出为 .txt",
    copied: "已复制",
    clearHistory: "清空历史",
...
function App() {
  ...
  const upsertMutation = useMutation({
    mutationFn: (sub: typeof newSub) => {
      ...
      const payload = { ...sub, id: editId, filters };
  ...
  const toggleMutation = useMutation({
    mutationFn: ({id, active}: {id: number, active: boolean}) => 
      invoke("toggle_subscription", { id, active }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['subscriptions'] });
    }
  });
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-slate-700 font-sans antialiased selection:bg-blue-100 h-screen flex flex-col overflow-hidden">
      <header className="bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center shrink-0">
        <div onClick={() => setView('dashboard')} className="flex items-center gap-2.5 cursor-pointer group">
          <Logo />
          <div><h1 className="text-sm font-bold tracking-tight text-slate-900">{t.title}</h1><p className="text-[9px] font-bold text-blue-500 uppercase tracking-widest leading-none">Desktop v1.0</p></div>
        </div>
        ...
        {view === 'dashboard' && (
          <>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold shadow-sm active:scale-95 text-[12px]"><Plus size={16} strokeWidth={3} /> {t.addTracker}</button>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {(subscriptions as any[])?.map((sub: any) => (
                    <tr key={sub.id} className={`hover:bg-slate-50/50 transition-colors group ${!sub.is_active ? 'opacity-50' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-[11px] text-slate-400 tabular-nums">{sub.last_checked_at || t.waiting}</td>
                      <td className="px-6 py-4 text-right"><div className="flex justify-end items-center gap-3">
                        <label className="relative inline-flex items-center cursor-pointer"><input type="checkbox" className="sr-only peer" checked={sub.is_active} onChange={() => toggleMutation.mutate({id: sub.id, active: !sub.is_active})} /><div className="w-9 h-5 bg-slate-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div></label>
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => { if(window.confirm(t.confirmClear)) clearHistoryMutation.mutate(); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-100 text-red-500 hover:bg-red-50 font-bold text-[11px]"><Trash2 size={14}/> {t.clearHistory}</button>
                <button onClick={() => copyToClipboard((historyList as any[])?.map((i:any) => i.magnet_link).join('\n') || '', 'batch')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold text-[11px] transition-all shadow-sm ${isBatchCopied ? 'bg-green-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{isBatchCopied ? <Check size={14}/> : <Copy size={14}/>} {isBatchCopied ? t.copied : t.copyAll}</button>
              </div>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">
              {(historyList as any[])?.map((item: any) => (
                <div key={item.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between gap-4 group">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider border ${item.status === 'submitted' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : item.status === 'skipped' ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>{item.status}</span>
                      <span className="text-[10px] text-slate-300 tabular-nums">{item.created_at}</span>
                    </div>
                    <h3 className="text-[13px] font-semibold text-slate-700 truncate pr-4">{item.title}</h3>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => copyToClipboard(item.magnet_link, item.id)} className={`p-2 rounded-lg transition-all ${copiedId === item.id ? 'bg-green-50 text-green-600' : 'text-slate-300 hover:bg-slate-100'}`}>{copiedId === item.id ? <Check size={14}/> : <Copy size={14}/>}</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {view === 'settings' && (
          <div className="animate-in fade-in duration-500">
            ...
            <div className="max-w-2xl bg-white p-8 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
              <div className="flex items-center gap-3 mb-10 text-slate-900 font-bold"><Globe size={20} className="text-blue-600" /> {t.ariaStatus}</div>
              <div className="space-y-6">
                <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1.5 block tracking-widest">{t.ariaRpc}</label><input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-blue-500 outline-none text-sm font-mono" value={editSettings.aria2_rpc_url} onChange={e => setEditSettings({...editSettings, aria2_rpc_url: e.target.value})} /></div>
                <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1.5 block tracking-widest">{t.ariaSecret}</label><input type="password" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-blue-500 outline-none text-sm font-mono" value={editSettings.aria2_rpc_secret} onChange={e => setEditSettings({...editSettings, aria2_rpc_secret: e.target.value})} /></div>
                <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100/50 text-[11px] font-medium text-blue-600 italic tracking-tight">{t.connectionTip}</div>
                <button onClick={() => saveSettingsMutation.mutate(editSettings)} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold text-xs shadow-sm transition-all">{t.saveSettings}</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-slate-200 p-6 space-y-6">
            <div className="flex justify-between items-center">
              <div><h3 className="font-bold text-xl text-slate-900">{editId ? t.editSub : t.newSub}</h3><p className="text-[10px] text-blue-500 font-bold uppercase tracking-widest">{t.configTracker}</p></div>
              <button onClick={closeModal} className="text-slate-300 hover:text-slate-900 text-2xl">×</button>
            </div>
            ...
            {!editId && (
              <div className={`flex items-center gap-4 p-4 rounded-2xl border transition-all cursor-pointer ${newSub.download_history ? 'bg-orange-50/50 border-orange-100' : 'bg-blue-50/50 border-blue-100'}`} onClick={() => setNewSub({...newSub, download_history: !newSub.download_history})}>
                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-blue-600 pointer-events-none" checked={newSub.download_history} readOnly />
                <div className="flex-1"><span className={`block text-xs font-bold uppercase ${newSub.download_history ? 'text-orange-900' : 'text-blue-900'}`}>{t.downloadHist}</span><p className="text-[9px] font-medium opacity-60 leading-none mt-1">{newSub.download_history ? t.histDesc : t.monitorDesc}</p></div>
              </div>
            )}
